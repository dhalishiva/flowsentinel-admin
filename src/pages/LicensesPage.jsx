...
// ── Key generator (same logic as generate-license.js but in browser) ─────────
...
export default function LicensesPage() {
  ...
  // HMAC secret — stored in portal settings or env
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">{licenses.length} licenses issued</p>
        </div>
        <button
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-6">
          ...
          {generatedKey ? (
            <div className="space-y-4">
              <div className="bg-slate-800 border border-indigo-700/50 rounded-xl p-5">
                ...
                <p className="font-mono text-sm text-indigo-300 break-all leading-relaxed">
                  {generatedKey.key}
                </p>
              </div>
            ...
            <form onSubmit={handleGenerate} className="space-y-5">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1.5">
                  HMAC Secret <span className="text-slate-600">(saved in browser — never sent to server)</span>
                </label>
                <input
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1.5">Customer name (for reference)</label>
                ...
                <div>
                  ...
                  <select value={form.license_type}
                    ...
                    <option value="standard">Standard (up to 5 mailboxes)</option>
                    <option value="professional">Professional (up to 15 mailboxes)</option>
                  </select>
                </div>
                <div>
                  ...
                  <input type="number" required min={1} max={50}
                    ...
                    onChange={e => setForm(f => ({ ...f, max_mailboxes: parseInt(e.target.value) }))}
                    className="w-full px-3 py-2 bg-slate-800 border border-slate-700 text-white rounded-lg outline-none focus:border-indigo-500 text-sm"
                  />
                </div>
                <div>
              ...
              <button type="submit" disabled={generating}
                className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg disabled:opacity-50 transition-colors">
                {generating ? <RefreshCw className="animate-spin" size={14} /> : <Key size={14} />}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-400">No licenses issued yet</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-800">
                  {['Tenant', 'Type', 'Mailboxes', 'Expires', 'Activated', 'Status'].map(h => (
              ...
              <tbody className="divide-y divide-slate-800">
                {licenses.map(l => (
                  <tr key={l.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 py-3">
                      {l.tenants ? (
                        <Link to={`/tenants/${l.tenant_id}`} className="hover:text-indigo-400 transition-colors">
                          <p className="text-sm font-medium text-white">{l.tenants.company_name}</p>
                          <p className="text-xs text-slate-500 font-mono">{l.tenants.company_code}</p>
                        </Link>
                      ) : (
                    ...
                    <td className="px-5 py-3">
                      <span className="text-sm text-slate-300 capitalize">{l.license_type}</span>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-sm text-slate-300">{l.max_mailboxes}</span>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-sm text-slate-300">{formatDate(l.expires_at)}</span>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-sm text-slate-400">
                        {l.activated_at ? formatDate(l.activated_at) : '—'}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`text-xs font-medium px-2 py-1 rounded-full ${licenseStatusColor(l)}`}>
